...
const About = () => {
  const GradientHeading = ({ title, size }) => {
    return (
      <div className="flex flex-col items-center py-8">
        <h3 className={`text-center  nunito_sans font-semibold ${size}`}>{title.toUpperCase()}</h3>
  ...
  return (
    <>
      <div className=" md:w-[80%] mx-auto px-4 py-12 md:py-16 bg-rgb(219, 234, 254)"> {/* Adjusted background color */}
        <GradientHeading title="About Us" size="text-4xl md:text-5xl" /> {/* Decreased font size */}
        <div
        ...
        >
          <div className="p-4 w-full text-center lg:text-start">
            <h2 className="text-2xl lg:text-3xl font-medium "> {/* Adjusted font size */}
              Welcome To LENS
            </h2>
